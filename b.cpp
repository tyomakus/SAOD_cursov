...
int main()
{
	...
	while (true) {
		...
		cout << "\t2. Print sorted (Heap sort)" << endl;
		cout << "\t3. Find by a key (Otdel Num)" << endl;
		cout << "\t4. Find in a tree (Birth Date)" << endl;
		cout << "\t5. Coding(Disabled now)" << endl;
		cout << "\tEsc. Leave " << endl;
		cout << "\t----------------------------------------------------------" << endl;
		...
		switch (enter) {
			...
			case 51:
				cout <<endl;
				SetConsoleCP(866);
				cout << "Type Num Otdel looking for:"<<endl;
			
				cin >> dep_num;
				if(dep_num>=0){
					search = BSearch(mas, dep_num);
					if(search == -1){
						cout << "Ñîòðóäíèêà èç òàêîãî îòäåëà íå ñóùåñòâóåò" << endl;
					}
					else{	
						SetConsoleCP(1251);
						do{
						...
						while(true);
						SetConsoleCP(866);
						cout <<endl <<endl<<"Íàéäåíî "<<search-search_start <<" ñîòðóäíèêîâ ("<< search_start<< " "<< search-1<<")"<<endl;
						SetConsoleCP(1251);
...
void BaseCoding(){
	...
	printf("\n");
	fclose(f);
	//ñîðòèðîâêà ïî êîëè÷åñòâó ïîâòîðåíèé ñèìâîëîâ
	bool b = true;
	while (b)
	{
		b = false;
...
void Huffman(int n){
	...
	else
	{
		...
		int j = Up(n, q); // Ïîèñê è âñòàâêà ñóììû 
		Huffman(n - 1);
		Down(n, j); // Äîñòðàèâàíèå êîäîâ
	}
}
...
int compare_name(char* a, char* b) {
	...
	strcpy(bufferA, a);
	strcpy(bufferB, b);
	//íàõîäèì ïîçèöèþ ïåðâîãî ïðîáåëà
	int space_pos_a = strchr(a,' ') - a;
	int space_pos_b = strchr(b,' ') - b;
	
	//ìèíèìàëüíîå êîëè÷åñòâî ñèìâîëîâ â ôàìèëèè 3
	int count =4;
	//åñëè ñðàâíèâàåìûå ôàìèëèè ðàçíûõ ðàçìåðîâ âûáèðàåì ìåíüøèé ðàçìåð äëÿ ñðàâíåíèé
	...
	//åñëè ñðàâíèâàåìûå ôàìèëèè ðàçíûõ ðàçìåðîâ âûáèðàåì ìåíüøèé
	if(space_pos_a > space_pos_b){
		return 1;
	}
	if(space_pos_a < space_pos_b){
	...
	//åñëè ôàìèëèè îäèíàêîâû, òî...
	//íàõîäèì ïîçèöèþ âòîðîãî ïðîáåëà
	strcpy(bufferA, a);
	strcpy(bufferB, b);
	...
	count = 4;
	//åñëè ñðàâíèâàåìûå èìåíà ðàçíûõ ðàçìåðîâ âûáèðàåì ìåíüøèé ðàçìåð äëÿ ñðàâíåíèé
	if(space_pos_a < space_pos_b){
		count = space_pos_a;
	}
	else{
	...
	//åñëè ñðàâíèâàåìûå èìåíà ðàçíûõ ðàçìåðîâ âûáèðàåì ìåíüøèé
	if(space_pos_a > space_pos_b){
		return 1;
	}
	if(space_pos_a < space_pos_b){
	...
	//åñëè èìåíà îäèíàêîâû, òî...
	//íàõîäèì ïîçèöèþ òðåòüåãî ïðîáåëà
	bufferA[space_pos_a]='a';
	bufferB[space_pos_b]='a';
	space_pos_a = strchr(bufferA,' ') - bufferA;
	space_pos_b = strchr(bufferB,' ') - bufferB;
	...
	count = 4;
	//åñëè ñðàâíèâàåìûå îò÷åñòâà ðàçíûõ ðàçìåðîâ âûáèðàåì ìåíüøèé ðàçìåð äëÿ ñðàâíåíèé
	if(space_pos_a < space_pos_b){
		count = space_pos_a;
	}
	else{
...
void HeapSort (int n, list **A) {
	...
	//ñîðòèðîâêà ïî ïåðâè÷íîìó êëþ÷ó - íîìåð îòäåëà
	int l=n/2, r=n;
	while (l+1>0) {
		HeapBuild (A, l, r);
		l--;
	...
	//ñîðòèðîâêà ïî âòîðè÷íîìó êëþ÷ó - ÔÈÎ
	//ñîðèðóåò îòäåëüíî âñå ÔÈÎ äëÿ êàæäîãî èç îòäåëîâ
	int offset = 0;
	int counter_max = 0;
	...
	//äîñîðòèðîâêà ïî ÔÈÎ äëÿ ñòðîê ñ ìàêñèìàëüíûì íîìåðîì îòäåëà
	l = counter_max / 2;
	r = counter_max + 1;
	while (l + 1 > 0) {
		HeapBuild2 (A, l, r, offset);
...
//     if (A[j]->data->Department < pivot->data->Department) {
//       i++;
//       list* temp = A[i];
//       A[i] = A[j];
//       A[j] = temp;
